...
function zv2_squad(int $uid,int $squadId=0,bool $finalize=true):array{
 ...
 if($finalize&&(int)$s['arrives_at']>0&&(int)$s['arrives_at']<=time()){
  ...
  if($home==="$x|$y"){$cargo=zv2_squad_cargo($id);$hold=$db->query("SELECT buildings FROM strongholds WHERE userid=$uid LIMIT 1")->fetch_assoc();$levels=pipe_nums($hold['buildings']);$storage=(int)($levels[4]??0);if($cargo['items']&&$storage>0){foreach($cargo['items']as$item)zv2_add_item($uid,$item['id'],$item['amount']);$db->query("DELETE FROM squad_cargo WHERE squad_id=$id");$events[]='Storage received '.array_sum(array_column($cargo['items'],'amount')).' carried items.';}elseif($cargo['items'])$events[]='No Storage building is available; the loot remains loaded on the squad.';else $events[]='The squad returned to the stronghold.';}
  else{$roll=($id*31+$x*7+$y*13)%4;if($roll===1){$item=(($x+$y)%2)?5:6;$take=zv2_add_cargo($id,$item,2);if($take)$events[]='Travel event: found an abandoned supply bag.';}elseif($roll===2){$victim=$db->query("SELECT s.id,s.name FROM squad_members m JOIN survivors s ON s.id=m.survivor_id WHERE m.squad_id=$id AND s.hp>0 ORDER BY s.fatigue,s.id LIMIT 1");if($victim&&$victim->num_rows){$v=$victim->fetch_assoc();$db->query("UPDATE survivors SET hp=GREATEST(0,hp-1),fatigue=LEAST(100,fatigue+8) WHERE id=".(int)$v['id']);$events[]='Travel event: roadside ambush; '.$v['name'].' was wounded.';}}elseif($roll===3)$events[]='Travel event: a distant horde forced a cautious detour.';else $events[]='Travel event: the route remained quiet.';}
