...
function zv2_squad(int $uid,int $squadId=0,bool $finalize=true):array{
 ...
 if($finalize&&(int)$s['arrives_at']>0&&(int)$s['arrives_at']<=time()){
  ...
  if($home==="$x|$y"){$cargo=zv2_squad_cargo($id);foreach($cargo['items']as$item)zv2_add_item($uid,$item['id'],$item['amount']);if($cargo['items']){$db->query("DELETE FROM squad_cargo WHERE squad_id=$id");$events[]='Delivered '.array_sum(array_column($cargo['items'],'amount')).' carried items to the stronghold.';}}
