...
if($action==='loot'){
 ...
 $lootPercent=(float)($techEffects['room_loot']??0)+(in_array($itemId,[1,2],true)?(float)($techEffects['salvage_bonus']??0):0);$bonus=$lootPercent>0?max(1,(int)floor($available*$lootPercent/100)):0;$taken=zv2_add_cargo($activeSquad['id'],$itemId,$available+$bonus);if($taken<=0)json_err('cargo_full','The squad cannot carry any more. Return to the stronghold or leave something behind.');$actual=min($available,$taken);$bonusTaken=max(0,$taken-$actual);$remaining=$available-$actual;$left=[];foreach(explode('&',(string)$room['inventory'])as$e){if($e==='')continue;$b=explode('|',$e);if((int)($b[0]??0)===$itemId){if($remaining>0)$left[]=$itemId.'|'.$remaining;}else$left[]=$e;}$esc=$db->real_escape_string(implode('&',$left));$db->query("UPDATE roommap SET inventory='$esc' WHERE id=$roomId");if($itemId===8)$db->query("UPDATE research_state SET points=LEAST(9999,points+".($actual*5).") WHERE userid=$uid");$cargo=zv2_squad_cargo($activeSquad['id']);$message='Packed '.$taken.'× '.zv2_item_name($itemId).' into squad cargo.';if($bonusTaken)$message.=' Research recovered '.$bonusTaken.' bonus item'.($bonusTaken===1?'':'s').'.';if($taken<$available+$bonus)$message.=' Capacity reached; some supplies remain.';if($itemId===8)$message.=' Electronics yielded '.($actual*5).' research points.';$me=$db->real_escape_string($message);$db->query("INSERT INTO squad_events(squad_id,event_type,message,created_at) VALUES(".(int)$activeSquad['id'].",'loot','$me',".time().")");json_out(['ok'=>true,'action'=>'loot','item'=>['id'=>$itemId,'name'=>zv2_item_name($itemId),'amount'=>$taken,'remaining'=>$remaining,'researchBonus'=>$bonusTaken],'cargo'=>$cargo,'message'=>$message]);
}
