...
$db->begin_transaction();try{
 ...
 $npcNames=zv2_npc_names($uid,10);$lead=$db->real_escape_string($name);$companionA=$db->real_escape_string($npcNames[0]);$companionB=$db->real_escape_string($npcNames[1]);$db->query("INSERT INTO survivors(userid,name,hp,max_hp,attack_stat,defense_stat,equipped_weapon) VALUES($uid,'$lead',14,14,4,2,10),($uid,'$companionA',12,12,3,2,NULL),($uid,'$companionB',11,11,5,1,NULL)");
 ...
 $recruitNames=array_slice($npcNames,2,8);$requestPool=[3,4,5,6,7,8];$used=["$px|$py"=>true];for($i=0;$i<8;$i++){do{if($i<4){$rx=max(1,min(50,$px+random_int(-6,6)));$ry=max(1,min(50,$py+random_int(-6,6)));}else{$rx=random_int(2,49);$ry=random_int(2,49);}$key="$rx|$ry";}while(isset($used[$key]));$used[$key]=true;$rn=$db->real_escape_string($recruitNames[$i]);$atk=random_int(2,5);$def=random_int(1,3);$required=$requestPool[($uid+$i+$atk*2+$def)%count($requestPool)];$db->query("INSERT INTO recruit_encounters(userid,x,y,name,attack_stat,defense_stat,required_item) VALUES($uid,$rx,$ry,'$rn',$atk,$def,$required)");}
