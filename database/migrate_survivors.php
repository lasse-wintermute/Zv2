...
require_once __DIR__ . '/../api/npc_names.php';
...
$users=$db->query('SELECT id,username FROM users');while($users&&($u=$users->fetch_assoc())){$uid=(int)$u['id'];$c=$db->query("SELECT COUNT(*) n FROM survivors WHERE userid=$uid")->fetch_assoc();if((int)$c['n']===0){$names=zv2_npc_names($uid,2);$base=$db->real_escape_string($u['username']);$companionA=$db->real_escape_string($names[0]);$companionB=$db->real_escape_string($names[1]);$db->query("INSERT INTO survivors(userid,name,hp,max_hp,attack_stat,defense_stat,equipped_weapon) VALUES($uid,'$base',14,14,4,2,10),($uid,'$companionA',12,12,3,2,NULL),($uid,'$companionB',11,11,5,1,NULL)");$db->query("INSERT INTO inventory(userid,item_id,amount) VALUES($uid,10,1) ON DUPLICATE KEY UPDATE amount=GREATEST(amount,1)");}}
