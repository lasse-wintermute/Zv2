...
export function createTutorial(el,opts={}){
 ...
 el.addEventListener('click',e=>{const b=e.target.closest('[data-tutorial-action]');if(!b||b.disabled)return;const action=b.dataset.tutorialAction;b.disabled=true;if(action==='advance')onAdvance();if(action==='close'||action==='dismiss')onDismiss();if(action==='restart')onRestart();});
 ...
 function show(d){data=d;open=true;const c=d.content||{};const finished=d.complete;el.innerHTML=`<aside class="tutorial-card"><header><div><small>FIELD MANUAL · ${Math.min(d.step+1,d.total)}/${d.total}</small><h2>${esc(c.title||'Tutorial')}</h2></div><button data-tutorial-action="close" aria-label="Close field manual permanently" title="Close permanently">×</button></header><p>${esc(c.body||'')}</p><div class="tutorial-objective"><small>OBJECTIVE</small><b>${esc(c.objective||'')}</b></div><div class="tutorial-actions">${finished?'<button data-tutorial-action="restart">Restart tutorial</button>':`<button class="tutorial-skip" data-tutorial-action="dismiss">Don\'t show again</button><button class="tutorial-next" data-tutorial-action="advance"${d.ready?'':' disabled'}>${d.ready?'Continue':'Objective pending…'}</button>`}</div></aside>`;el.classList.add('open');focus(c.target);}
 function hide(){open=false;el.classList.remove('open');clearFocus();}
 function update(d){data=d;if(!d.dismissed&&(d.active||d.complete))show(d);else hide();}
 function reopen(){if(data&&!data.dismissed&&(data.active||data.complete))show(data);else onRestart();}
