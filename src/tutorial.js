...
export function createTutorial(el,opts={}){
 ...
 function focus(target){dragger.restore();clearFocus();const selector=target==='hud'?'#hud':target==='world'?'#modebtn':target==='squads'?'#forcesbtn':target==='compound'?'#game':'';if(selector)document.querySelector(selector)?.classList.add('tutorial-focus');}
